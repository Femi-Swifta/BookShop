package com.femtech.book_network.book;

import com.femtech.book_network.common.PageResponse;
import com.femtech.book_network.exception.OperationNotPermittedException;
import com.femtech.book_network.file.FileStorageService;
import com.femtech.book_network.history.BookTransactionHistory;
import com.femtech.book_network.history.BookTransactionHistoryRepository;
...
import static com.femtech.book_network.book.BookSpecification.withOwnerId;
...
public class BookService {
    ...
    public PageResponse<BookResponse> findAllBooksByOwner(int page, int size, Authentication connectedUser) {
         User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        Page<Book> books = bookRepository.findAll(withOwnerId(user.getName()), pageable);
        List<BookResponse> booksResponse = books.stream()
                .map(bookMapper::toBookResponse)
                .toList();
        return new PageResponse<>(
    ...
    public PageResponse<BorrowedBookResponse> findAllReturnedBooks(int page, int size, Authentication connectedUser) {
         User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        Page<BookTransactionHistory> allBorrowedBooks = transactionHistoryRepository.findAllReturnedBooks(pageable, Integer.valueOf(user.getName()));
        List<BorrowedBookResponse> booksResponse = allBorrowedBooks.stream()
                .map(bookMapper::toBorrowedBookResponse)
                .toList();
        return new PageResponse<>(
